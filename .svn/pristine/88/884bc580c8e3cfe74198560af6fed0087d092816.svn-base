...
var OutletForm = function (_BaseForm) {
    ...
    createClass(OutletForm, [{
        ...
        value: function initDialog() {
            ...
            tablePane.addColumns([{ tag: 'switch_name', name: 'switch_name', displayName: 'Switch Name', accessType: 'attr' }, { tag: 'switch_ip', name: 'switch_ip', displayName: 'Switch IP', accessType: 'attr' }, { tag: 'switch_port_no', name: 'switch_port_no', displayName: 'Port No', accessType: 'attr' }, { tag: 'switch_ifindex', name: 'switch_ifindex', displayName: 'Switch Ifindex', accessType: 'attr' }, { tag: 'switch_mac_address', name: 'switch_mac_address', displayName: 'Switch Mac', accessType: 'attr' }, { tag: 'switch_asset_tag', name: 'switch_asset_tag', displayName: 'Switch Asset No', accessType: 'attr' }, { tag: 'access_begin', name: 'access_begin', displayName: 'Access Begin', accessType: 'attr' }, { tag: 'access_end', name: 'access_end', displayName: 'Access End', accessType: 'attr' }, { tag: 'enduser_name', name: 'enduser_name', displayName: 'Enduser Name', accessType: 'attr' }, { tag: 'enduser_mac_address', name: 'enduser_mac_address', displayName: 'Enduser Mac', accessType: 'attr' }, { tag: 'enduser_ip_address', name: 'enduser_ip_address', displayName: 'Enduser IP', accessType: 'attr' }, { tag: 'enduser_asset_tag', name: 'enduser_asset_tag', displayName: 'Enduser Asset No', accessType: 'attr' }]);
			...
			util.ajax({
				...
				success: function success(r) {
					...
					if (r && r.length > 0) {
						r.forEach(function (d) {
							if (d.idx === 1) {
								if (_this.globaledge instanceof ht.Edge) dm.remove(_this.globaledge);
								var node = cms.getTreeNodeByNodeid(d.node_id),
								    parent = node.getParent(),
								    typeName = cms.getNodeTypeNameById(parent.a('type'));

								if (typeName === RACK$3) {
									_this.globaledge = new ht.Edge(_this.getDataByTreeData(parent), ld);
								} else {
									_this.globaledge = new ht.Edge(_this.getDataByTreeData(node), ld);
								}
								/*			                edge.s("flow", true);
        ...
        */
        ...
        this.globaledge.s("flow.count", 6);
                    ...
                    this.globaledge.s("flow.element.shadow.visible", false);*/
								...
								_this.globaledge.s("edge.type", "ortho");
								_this.globaledge.s("edge.width", 5);
								_this.globaledge.s("edge.color", "#66FF00");
								_this.globaledge.s("edge.center", true);
								_this.globaledge.s("edge.corner.radius", 15);
								dm.add(_this.globaledge);
								//this.gv.dm();
							}
       ...
       if (d.connectorimage) {
		...
		value: function addListener() {
			...
			self.cms.notifier.add(function (e) {
				if (e.kind === 'treeAdd' && _this2.cms.splitView.getRightView() === _this2.getHTView()) {
					_this2.addNode(e.data, _this2.gv.dm().getDataByTag('background'));
				}
			});
				...
				this.gv.mi(function (e) {
					...
					if (e.kind === 'doubleClickData') {
						...
						if (ld) {
							...
							_this2.showComponentDetail(util.getNodeId(ld), ld.getImage());
						} else {
							_this2.propertyView.getView().style.display = 'none';
						}
					} else if (e.kind === 'clickData' && !isSpaceDown) {
						var _ld = _this2.gv.sm().ld();

						if (_ld) {
							if (_this2.globalConnection !== _ld) {
								var _typeName = cms.getNodeTypeNameById(_ld.a('type'));
								if (_typeName === OUTLET$2) {
									_this2.showConnectionDiagram(_ld, util.getNodeId(_ld));
									_this2.globalConnection = _ld;
       ...
       action: function() {
		...
		value: function filterInvalid() {
			...
			var dm = this.gv.dm(),
				...
				return !_this3.cms.getTreeNodeByNodeid(util.getNodeId(data.getTag() === 'background' ? _this3.data : data));
			});
			invalidList.each(function (data) {
				dm.remove(data);
			});
		...
		value: function setData(data) {
			...
			this.cms.loadNodeDetail(data, function (r) {
				if (r && r[0]) {
					...
					if (json) {
						_this4.gv.dm().deserialize(json);
						var background = _this4.gv.dm().getDataByTag('background');
						if (background) {
							background.a(_this4.data.getAttrObject());
						}
						_this4.filterInvalid();
						if (_this4.cms.isViewing) {
							_this4.gv.dm().each(function (d) {
								/*d.s({
        	'select.color': '#014288',
        	'select.width': 3
        })*/
								var typeName = _this4.cms.getNodeTypeNameById(d.a('type'));
								if (typeName === AREA$3) {
									/*d.s({
         	'opacity': 0
         })*/
				...
				setTimeout(function () {
					if (_this4.alarm) {
						_this4.alarm.start(util.getNodeId(_this4.data));
					}
					if (_this4.cms.isViewing) {
						_this4.fitSelection();
					} else {
						_this4.gv.fitContent(1);
					}
				}, 100);
			});
		}
			...
			if (!self.fileDom) {
				...
				fileDom.addEventListener('change', function (e) {
					...
					if (file && file.type.indexOf('image') === 0) {
						/*
      loading.show();
      self.cms.uploadFloorMapImage(data, r => {
      	loading.hide();
      	if (r) {
      		tipWin.show(r.Result);
      		if(r.Result==='Success'){
      			background.setImage("/resources/"+file.name);
			...
			if (cms.getNodeTypeNameById(ld.a('type')) === AREA$3) {
				...
				if (e.kind === 'beginEditResize') {
					...
					ld.getChildren().each(function (d) {
						_this6.areaChildren.push(d.getPosition());
						if (cms.getNodeTypeNameById(d.a('type')) === RACK_SPACE$3) {
							d.getChildren().each(function (dd) {
								_this6.areaChildrenRect.push(dd.getRect());
							});
						} else {
							_this6.areaChildrenRect.push(d.getRect());
						}
					});
					...
					ld.getChildren().each(function (d) {
						{
							d.setPosition(_this6.areaChildren[i++]);
						}
					});
