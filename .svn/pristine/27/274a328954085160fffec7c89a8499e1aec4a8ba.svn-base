    ...
    createClass(CMS, [{
        ...
        value: function readCSV(text) {
            ...
            var rows = text.split('\n'),
                ...
                listBasis = [];
            var dt = new Date();
            this.form.getItemById('textArea').element.getElement().value = 'Started @' + dt.toLocaleString() + '\n=============================';
            rows.forEach(function (row) {
                var last = blocks[blocks.length - 1];
                if (row[0] === '#') {
                    var basisName = row.replace(/#|\,|\n|\r/g, '');
                    blocks.push(basisName ? ['' + BASIS_FLAG + basisName] : []);
                    listBasis.push(basisName);
                    return;
                }
                last.push(row);
            });
            blocks.forEach(function (block) {
                ...
                if (block.length > 0) {
                    _this._validateBlock(block, listBasis.slice());
                }
            });

            this.form.getItemById('textArea').element.getElement().value += '\n=============================\n' + 'Ended @' + dt.toLocaleString();
        }
    }, {
        key: '_removeA',
        value: function _removeA(arr) {
            var what,
                a = arguments,
                L = a.length,
                ax;
        ...
        key: '_validateBlock',
        value: function _validateBlock(rows, list) {
            var basisNodeName = void 0;

            if (BASIS_FLAG === rows[0].substring(0, 3)) {
                basisNodeName = rows[0].replace(BASIS_FLAG, '');
                this._removeA(list, basisNodeName);
                var flag = false;
                for (var i = 1; i < rows.length; i++) {
                    var cols = rows[i].split(',');
                    if (cols.indexOf(basisNodeName) >= 0) {
                        flag = true;
                        break;
                    }
                }
                if (!flag) {
                    this.form.getItemById('textArea').element.getElement().value += '\n' + 'Invalid basis defined on [' + basisNodeName + ']';
                }

                for (var _i = 1; _i < rows.length; _i++) {
                    var items = rows[_i].split(',');
                    for (var j = 0; j < items.length; j++) {
                        if (list.includes(items[j])) {
                            //alert('Invalid item ' + items[j] + ' on ' + basisNodeName + ' line '+i);
                            this.form.getItemById('textArea').element.getElement().value += '\n' + 'Invalid item ' + items[j] + ' on [' + basisNodeName + ' @' + _i + ']';
                        }
                        //device,port,cable
                        if (j > 1 && items[j - 2] !== '' && isNaN(items[j - 2]) && items[j - 1] !== '' && !isNaN(items[j - 1]) && items[j] !== '' && !isNaN(items[j])) {
                            if (!this.cableCIIDList.includes(items[j])) {
                                //alert('Invalid item ' + items[j] + ' on ' + basisNodeName + ' line '+i);
                                this.form.getItemById('textArea').element.getElement().value += '\n' + 'Invalid Cable ' + items[j] + ' on [' + basisNodeName + ' @' + _i + ']';
                            }
                        }
                        //cable,device,port
                        else if (j < items.length - 1 && items[j] !== '' && !isNaN(items[j]) && items[j + 1] !== '' && isNaN(items[j + 1]) && items[j + 2] !== '' && !isNaN(items[j + 2])) {
                                if (!this.cableCIIDList.includes(items[j])) {
                                    //alert('Invalid item ' + items[j] + ' on ' + basisNodeName + ' line '+i);
                                    this.form.getItemById('textArea').element.getElement().value += '\n' + 'Invalid Cable ' + items[j] + ' on [' + basisNodeName + ' @' + _i + ']';
                                }
        ...
        value: function loadCableData() {
            var cableCIIDList = this.cableCIIDList = [];
            util.ajax({
                url: API_HEAD + 'getCableIcon',
                success: function success(r) {
                    r.forEach(function (d) {
                        cableCIIDList.push(d.id);
                    });
                }
            });
        }
        ...
        value: function init() {
            ...
            /*var exportJsonTextArea = form.getItemById('textArea').element.getElement();
