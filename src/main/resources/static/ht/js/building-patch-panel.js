...
var nodeid = parseInt(util.getUrlParamter('nodeid'));
var ppnodename = void 0;
var siteid = parseInt(util.getUrlParamter('siteid'));
    ...
    createClass(CMS, [{
        ...
        value: function initUI() {
            ...
            var self = this,
                ...
                toolbarTree = this.toolbarTree = new Toolbar(32, 8, 8),
                rightBorderPane = this.rightBorderPane = new ht.widget.BorderPane(),
                cablePane = this.cablePane = new ht.widget.Palette(),
                splitView = this.splitView = new ht.widget.SplitView(leftBorderPane, rightBorderPane, 'h', 280),
            ...
            leftBorderPane.setTopView(toolbarTree);
            leftBorderPane.setTopHeight(toolbarTree.toolbarHeight);
            ...
            self.editInteractors = [scrollBarinteractor, selectInteractor, moveInteractor, defaultInteractor, touchInteractor];
            self.uneditInteractor = [selectInteractor, defaultInteractor];
            ...
            treeView.getLabelColor = function (data) {
                var typeName = self.getNodeTypeNameById(data.a('type'));
                if (typeName === PATCH_PANEL) {
                    return util.getNodeId(data) === nodeid ? '#FF9900' : '#000';
                } else if (data.a('port')) {
                    return !data.a('isPlugged') ? '#000' : '#aaa';
                } else {
                    return '#aaa';
                    // return '#00ff02';
                }
            ...
            contextMenu.beforeShow = function (e) {
                ...
                if (data instanceof Patch || !_this._editable) {
            ...
            contextMenu.addTo(gv.getView());
        }
    }, {
        key: 'handlerSearchOnTree',
        value: function handlerSearchOnTree() {
            var toolbarTree = this.toolbarTree,
                treeView = this.treeView,
                list = matchHelper.list,
                key = matchHelper.key,
                keyword = toolbarTree.v('keyword'),
        ...
        value: function removeSelection() {
            ...
            selection.each(function (data) {
                ...
                var customId = data.a('customId');
                if (customId) {
                    _this2.getNodeOnTreeByCustomId(customId).a('isPlugged', false);
                }
        ...
        value: function initToolbar() {
            ...
            var toolbarItems = [{
                ...
                action: function action() {
                    ...
                    util.exportAndDownload2(gvFloor.toDataURL(), 'PP-' + ppnodename + '.png');
                }
            }, {
                ...
                icon: self.getToolbarIcon('toolbar.import.csv', function () {
                    return self._editable;
                }),
        ...
        value: function handleSave() {
            ...
            loading.show();
            var linkage = this.patchPanel.getLinkageElements(),
                data = {
                linkedelements: linkage[0],
                invalid: linkage[2],
                // json_data:'1',
                patchpanels: linkage[1],
                switchtopo: linkage[3]
            };
            if (this.modelId) {
            ...
            util.ajax({
                url: API_HEAD + 'saveLinkageConnectionDetail',
                type: 'post',
                dataType: 'json',
        ...
        value: function handleRemove() {
            ...
            if (confirm('Are you sure you want to delete ?')) {
                ...
                if (this.modelId) {
                    util.ajax({
                        url: API_HEAD + 'removeLinkageConnection/' + this.modelId,
                        success: function success(r) {
                            delete _this4.modelId;
                        }
                    });
                }
            }
        ...
        key: 'clearLinkage',
        value: function clearLinkage() {
            var dm = this.gv.dm(),
                list = dm.toDatas();
            this.patchPanel.reset();
            list.remove(this.patchPanel);
            list.reverseEach(function (node) {
                dm.remove(node);
            });
            this.treeView.dm().each(function (data) {
                if (data.a('customId')) {
                    data.a('isPlugged', false);
                }
            });
        }
    }, {
        key: 'setEditable',
        value: function setEditable(v) {
        ...
        key: 'getToolbarIcon',
        value: function getToolbarIcon(url, stateFunc, basisFunc) {
            return {
                width: ICON_SIZE + 6,
                height: ICON_SIZE + 6,
                "blendMode": "override",
                    ...
                    color: {
                        func: function func() {
                            if (basisFunc) {
                                var basis = basisFunc();
                                var state = stateFunc();
                                return basis ? state ? '#0F0' : '#ababab' : '#ababab';
                            }
                            if (stateFunc) {
                                var state = stateFunc();
        ...
        value: function loadTreeData() {
            ...
            util.ajax({
                ...
                success: function success(r) {
                    ...
                    _this5.loadProductLinkageImage();
                    if (nodeid) {
                        _this5.loadPatchPanel();
                        _this5.loadDevicePorts();
                    }
            ...
            list.forEach(function (item, index) {
                util.ajax({
                    ...
                    success: function success(ret) {
                        _this7._productMap[item.type] = ret;
        ...
        key: 'loadDevicePorts',
        value: function loadDevicePorts() {
            var _this8 = this;
            ...
            var dm = this.treeView.dm(),
                ...
                usedList = [];
            // set used
            this.gv.dm().each(function (node) {
                if (node.a('customId')) {
                    usedList.push(node.a('customId'));
                }
            });

            dm.each(function (data) {
                var typeName = _this8.getNodeTypeNameById(data.a('type')),
                    nid = util.getNodeId(data);

                // if (parseInt(nodeid) === nid) return;
                if (list.indexOf(typeName) >= 0) {
                    var totalPort = data.a('totalPort');
                    if (totalPort) {
                        _this8.addDevicePorts(data, totalPort, usedList);
                    } else {
                        _this8.loadNodeDetail(data, function (detail) {
                            totalPort = detail[0].total_port;
                            data.a('totalPort', totalPort);
                            _this8.addDevicePorts(data, totalPort, usedList);
                        });
                    }
                }
        ...
        key: 'addDevicePorts',
        value: function addDevicePorts(device, totalPort, usedList) {
            var nid = util.getNodeId(device),
                dm = this.treeView.dm();
            if (nid === nodeid) {
                var curr = this.getNodeOnTreeByNodeid(nodeid);
                if (curr.getChildren()) {
                    curr.getChildren().reverseEach(function (child) {
                        dm.remove(child);
                    });
                }
                return;
            }
            if (device.getChildren().size() > 0) return;

            for (var i = 1; i <= totalPort; i++) {
                var treeNode = new ht.Data(),
                    customId = nid + '-' + i;
                treeNode.setName('port' + i);

                treeNode.a({
                    nodeid: nid,
                    customId: customId,
                    port: i,
                    isPlugged: usedList.indexOf(customId) >= 0
                });

                treeNode.setParent(device);
                dm.add(treeNode);
            }
        }
    }, {
        ...
        value: function loadPatchPanel() {
            ...
            util.ajax({ // 
                ...
                success: function success(r) {
                    loading.hide();
                    _this9.loadSiteTreeNodeDetail(function (info) {
                        if (r) {
                            //if (r.id) 
                            {
                                _this9.modelId = r.id;
                            }
                            _this9.buildLinkageByElements(r.linkedelements, r.invalid);
                            if (r.is_basis != undefined) {
                                _this9.is_basis = r.is_basis;
                            } else {
                                _this9.is_basis = true;
                            }
                            _this9.setEditable(false);
                            _this9.initToolbar();
                        }
                    });
                },
        ...
        value: function buildLinkageByElements(linkedelements, invalid) {
            ...
            if (linkedelements && linkedelements.length > 0) {
                ...
                validList.forEach(function (line) {
                    ...
                    line.r.forEach(function (d, idx) {
                        if (d.portname) {
                            var device = _this10.getNodeOnTreeByNodeid(d.nodeid);
                            rect = _this10.patchPanel.getCellRectByIndex(idx + 1, port, true, device);
                            node = _this10.createDevicePort2(d, device.getName(), parseInt(d.portname));
                        } else {
                            rect = _this10.patchPanel.getCellRectByIndex(idx + 1, port);
                            node = _this10.createCable(_this10.getCableByCategoryId(d.connector_id || d.category_id));
                        }
                        if (rect) {
                            dm.add(node);
                            _this10.patchPanel.plugIn(node, rect);
                        }
                    });
                    ...
                    line.l.reverse().forEach(function (d, idx) {
                        if (d.portname) {
                            var device = _this10.getNodeOnTreeByNodeid(d.nodeid);
                            rect = _this10.patchPanel.getCellRectByIndex(-1 - idx, port, true, device);
                            node = _this10.createDevicePort2(d, device.getName(), parseInt(d.portname));
                        } else {
                            rect = _this10.patchPanel.getCellRectByIndex(-1 - idx, port);
                            node = _this10.createCable(_this10.getCableByCategoryId(d.connector_id || d.category_id));
                        }
                        if (rect) {
                            dm.add(node);
            ...
            if (invalid && invalid.length > 0) {
                invalid.forEach(function (d) {
                    ...
                    if (d.nodeid) {
                        var device = _this10.getNodeOnTreeByNodeid(d.nodeid);
                        rect = _this10.patchPanel.getCellRectByIndex(d.x, d.y, true, device);
                        node = _this10.createDevicePort2(d, device.getName(), parseInt(d.port === undefined ? d.portname : d.port));
                    } else {
                        rect = _this10.patchPanel.getCellRectByIndex(d.x, d.y);
                        node = _this10.createCable(_this10.getCableByCategoryId(d.category_id));
                    }
                    if (rect) {
                        dm.add(node);
                        _this10.patchPanel.plugIn(node, rect);
            ...
            setTimeout(function () {
                _this10.resetTreePortAvailable();
            }, 1500);
        }
    }, {
        key: 'resetTreePortAvailable',
        value: function resetTreePortAvailable() {
            var _this11 = this;

            this.treeView.dm().each(function (td) {
                if (td.a('customId')) {
                    var gvd = _this11.gv.dm().toDatas(function (d) {
                        return d.a('customId') === td.a('customId');
                    });
                    td.a('isPlugged', gvd.size() > 0);
                }
            });
        }
    }, {
        key: 'loadSiteTreeNodeDetail',
        value: function loadSiteTreeNodeDetail(callback) {
            var _this12 = this;
            ...
            util.ajax({
                ...
                success: function success(r) {
                    ...
                    if (r && r[0]) {
                        var obj = r[0];
                        var patchPanel = _this12.patchPanel = new Patch(),
                            treeNode = _this12.getNodeOnTreeByNodeid(nodeid);
                        patchPanel.a({
                            'nodeid': nodeid,
                            'port.count': obj.total_port,
                            'deviceName': treeNode.getName()
                        });
                        ppnodename = treeNode.getName();
                        patchPanel.setCellSizeCable(50, 42);
                        patchPanel.setCellSizeDevice(100, 42);
                        _this12.gv.dm().add(patchPanel);
                        _this12.gv.fitContent();
                        if (callback) {
                            callback(r[0]);
                        }
                    }
        ...
        key: 'getNodeOnGvByCustomId',
        value: function getNodeOnGvByCustomId(customId) {
            var datas = this.gv.dm().getDatas(),
                size = datas.size();

            for (var i = 0; i < size; i++) {
                if (datas.get(i).a('customId') === customId) {
                    return datas.get(i);
                }
            }
        }
    }, {
        key: 'getNodeOnTreeByCustomId',
        value: function getNodeOnTreeByCustomId(customId) {
            var dm = this.treeView.dm(),
                size = dm.size(),
                list = dm.getDatas();
            customId = customId.replace(/\r/g, '');

            for (var i = 0; i < size; i++) {
                var data = list.get(i);
                if (data.a('customId') === customId) {
                    return data;
                }
            }
        }
    }, {
        key: 'getNodeOnTreeByNodeid',
        value: function getNodeOnTreeByNodeid(nodeid) {
            var dm = this.treeView.dm(),
                size = dm.size(),
                list = dm.getDatas();
        ...
        key: 'getNodeTypeNameByNodeid',
        value: function getNodeTypeNameByNodeid(nodeid) {
            var node = this.getNodeOnTreeByNodeid(nodeid);
            return this.getNodeTypeNameById(node.a('type'));
        }
    }, {
        ...
        key: 'getNodeidByDeviceName',
        value: function getNodeidByDeviceName(deviceName) {
            var dm = this.treeView.dm(),
                size = dm.size(),
                list = dm.getDatas();

            for (var i = 0; i < size; i++) {
                if (list.get(i).a('name') === deviceName) {
                    return util.getNodeId(list.get(i));
                }
            }
        }
    }, {
        key: 'getCableByCIID',
        value: function getCableByCIID(name) {
            var dm = this.cablePane.dm(),
                size = dm.size(),
                list = dm.getDatas();

            for (var i = 0; i < size; i++) {
                var data = list.get(i);
                /*if (!(data instanceof ht.Group) && data.getName().toLowerCase() === name.toLowerCase()) {
                    return data;
                }*/
                if (!(data instanceof ht.Group) && data.a('id') == name) {
                    return data;
                }
            }
        }
    }, {
        key: 'getCableByCategoryId',
        value: function getCableByCategoryId(id) {
            var list = this.cablePane.dm().toDatas(function (d) {
                return d.a('id') === id;
            });
        ...
        key: 'handleTreeDND',
        value: function handleTreeDND(e, state) {
            var target = this.treeView.getDataAt(e);
            if (state === 'prepare') {
                if (target.a('type') && this.getNodeTypeNameById(target.a('type')) === PATCH_PANEL) {
                    this._treeDNDMode = 'patch';
                } else {
                    this._treeDNDMode = 'port';
                }
            }
            if (this._treeDNDMode === 'patch') {
                this.hanlePatchPanel(e, state);
            } else {
                this.handlePortDND(e, state);
            }
        }
    }, {
        key: 'handlePortDND',
        value: function handlePortDND(e, state) {
            if (!this._editable) return;
            ...
            if (state === 'prepare') {
                ...
                if (!data || !data.a('port')) return;

                treeView.sm().ss(data);

                if (data.a('isPlugged')) {
                    this.gv.sm().ss(this.getNodeOnGvByCustomId(data.a('customId')));
                    return;
                }
                this.createDND(data.getParent().getName() + ' - ' + data.a('port'));
            }
            if (!dnd) return;
            ...
            if (state === 'begin') {
                ...
                this.showDragHelper(e, true, ld);
            } else if (state === 'end') {
                var ld = treeView.sm().ld(),
                    nid = ld.a('nodeid'),
                    rect = this.showDragHelper(e, true, ld);

                if (rect) {
                    var device = this.getNodeOnTreeByNodeid(ld.a('nodeid'));
                    var node = this.createDevicePort(ld, device.getName(), ld.a('port'));
                    dm.add(node);
                    this.patchPanel.plugIn(node, rect);

                    dm.remove(this.dragHelper);
                    this.dirty = true;
                }
                this.clearDND();
            }
        ...
        key: 'hanlePatchPanel',
        value: function hanlePatchPanel(e, state) {
            var treeView = this.treeView,
                dnd = this.dnd,
                dm = this.gv.dm();
            if (state === 'prepare') {
                var pp = treeView.getDataAt(e);
                if (nodeid === util.getNodeId(pp)) return;
                this.treeView.sm().ss(pp);
                this.createDND(pp.getName());
            }
            if (!dnd) return;
            ...
            if (state === 'begin') {
                ...
                if (ht.Default.containedInView(e, this.gv)) {
                    if (this.dirty) {
                        if (confirm('Are you sure you want to switch patch panel before saving?')) {
                            this.switchPatchPanel(this.treeView.sm().ld());
                            this.dirty = false;
                        }
                    } else {
                        this.switchPatchPanel(this.treeView.sm().ld());
        ...
        key: 'createDND',
        value: function createDND(text) {
            var dnd = this.dnd;
            if (dnd && dnd.parentNode) {
                document.body.removeChild(dnd);
            }
            dnd = this.dnd = ht.Default.createDiv();
            dnd.style.zIndex = 10;
            dnd.innerText = text;
        }
    }, {
        key: 'clearDND',
        value: function clearDND() {
            document.body.removeChild(this.dnd);
            this.dnd = null;
        }
    }, {
        key: 'handleCableDND',
        value: function handleCableDND(e, state) {
            if (!this._editable) return;
            var self = this,
                cablePane = self.cablePane,
                dm = self.gv.dm();
        ...
        value: function handleInteractor(e) {
            ...
            if (e.kind === 'prepareMove') {
                ...
                this.showDragHelper(e.event, target.a('device.type') === 1, target);
                this.gv.dm().sendToTop(target);
            } else if (e.kind === 'endMove') {
                var rect = this.showDragHelper(e.event, target.a('device.type') === 1, target);
                if (rect) {
        ...
        key: 'showDragHelper',
        value: function showDragHelper(e, isPort, nid) {
            var self = this,
            ...
            if (data) {
                var rect = data.getCellRect(self.gv.lp(e), isPort, nid);
                if (rect) {
        ...
        key: 'readCSV',
        value: function readCSV(text) {
            var _this13 = this;

            var rows = text.split('\n'),
                targetPatch = this.getNodeOnTreeByNodeid(nodeid),
                targetDeviceName = targetPatch.getName(),
                blocks = [[]];
            ...
            rows.forEach(function (row) {
                ...
                if (row[0] === '#') {
                    var basisId = _this13.getNodeidByDeviceName(row.replace(/#|\,|\n|\r/g, ''));
                    blocks.push(basisId ? ['' + BASIS_FLAG + basisId] : []);
                    return;
                }
                last.push(row);
            });
            blocks.forEach(function (block) {
                ...
                for (var i = 0; i < block.length; i++) {
                    var cols = block[i].split(',');
                    if (cols.indexOf(targetDeviceName) >= 0) {
                        flag = true;
                        break;
                    }
                }
                if (block.length > 0) {
                    _this13._readCSV(block, flag);
                }
            });
        }
    }, {
        key: '_readCSV',
        value: function _readCSV(rows, isBasis) {
            var _this14 = this;
            ...
            var linkedelements = [],
                ...
                basisNodeid = void 0,
                basisIndex = void 0;
            if (!isBasis && BASIS_FLAG === rows[0].substring(0, 3)) {
                basisNodeid = parseInt(rows[0].replace(BASIS_FLAG, ''));
                patchpanels.push(basisNodeid);

                var device = this.getNodeOnTreeByNodeid(basisNodeid),
                    deviceName = device.getName();
                for (var i = 1; i < rows.length; i++) {
                    var items = rows[i].split(',');
                    for (var j = 0; j < items.length; j++) {
                        if (items[j] === deviceName) {
                            basisIndex = j;
                            break;
                        }
                    }
                    if (basisIndex !== null && basisIndex !== undefined) {
                        break;
                    }
                }
            } else {
                for (var _i = 0; _i < rows.length; _i++) {
                    var _items = rows[_i].split(',');
                    for (var _j = 0; _j < _items.length; _j++) {
                        var nid = this.getNodeidByDeviceName(_items[_j]);
                        if (nid && nid === this.patchPanel.a('nodeid')) {
                            basisIndex = _j;
                            basisNodeid = nid;
                            break;
                        }
            ...
            rows.forEach(function (line) {
                if (!line || !util.trim(line) || BASIS_FLAG === util.trim(line).substring(0, 3)) return;
                var items = line.split(','),
                    list = [],
                    skip = 0,
                    continuous = true,
                    port = parseInt(items[basisIndex + 1]),
                    x = 0,
                    item = void 0;

                for (var _i2 = basisIndex; _i2 < items.length; _i2++) {
                    item = items[_i2];
                    if (_i2 === basisIndex) {
                        list.push({ nodeid: basisNodeid, portname: port, x: x, y: port - 1 });
                        _i2++;
                    } else if (item && util.trim(item)) {
                        if ((_i2 - basisIndex) % 3 === 0) {
                            // device
                            var _nid = _this14.getNodeidByDeviceName(item);
                            if (_nid) {
                                list.push({
                                    nodeid: _nid,
                                    portname: parseInt(items[_i2 + 1]),
                                    x: x,
                                    y: port - 1
                                });
                                if (skip > 0) {
                            ...
                            _i2++;
                        } else if ((_i2 - basisIndex - 1) % 3 === 1) {
                            // cable
                            var cable = _this14.getCableByCIID(item);
                            if (cable) {
                                list.push({
                                    category_id: cable.a('id'),
                                    x: x,
                                    y: port - 1
                                });
                                if (skip > 0) {
                ...
                for (var _i3 = basisIndex - 1; _i3 >= 0; _i3--) {
                    item = items[_i3];
                    if (item && util.trim(item)) {
                        if ((basisIndex - _i3) % 3 === 2) {
                            // device
                            var _nid2 = _this14.getNodeidByDeviceName(items[_i3 - 1]);
                            if (_nid2) {
                                list.unshift({
                                    nodeid: _nid2,
                                    portname: parseInt(item),
                                    x: x,
                                    y: port - 1
                                });
                                if (skip > 0) {
                            ...
                            _i3--;
                        } else if ((basisIndex - _i3) % 3 === 1) {
                            // cable
                            var _cable = _this14.getCableByCIID(item);
                            if (_cable) {
                                list.unshift({
                                    category_id: _cable.a('id'),
                                    x: x,
                                    y: port - 1
                                });
                                if (skip > 0) {
                ...
                if (!continuous) {
                    var foundswitchtopo = false,
                        ...
                        _j2 = 0;
                    //get the switch topology
                    for (; _j2 + 2 < list.length;) {
                        var a = list[_j2],
                            c = list[_j2 + 1],
                            b = list[_j2 + 2];

                        if (a.nodeid && (_this14.getNodeTypeNameByNodeid(a.nodeid) === BACKBONE || _this14.getNodeTypeNameByNodeid(a.nodeid) === L3_SWITCH || _this14.getNodeTypeNameByNodeid(a.nodeid) === L2_SWITCH) && b.nodeid && (_this14.getNodeTypeNameByNodeid(b.nodeid) === BACKBONE || _this14.getNodeTypeNameByNodeid(b.nodeid) === L3_SWITCH || _this14.getNodeTypeNameByNodeid(b.nodeid) === L2_SWITCH) && c.category_id) {
                            //if((a.x+1 === c.x) && (c.x+1 === b.x))
                            {
                                switchtopos.push({
                                    interface_a: { nodeid: a.nodeid, portname: a.portname },
                        ...
                        } else if (a.nodeid && (_this14.getNodeTypeNameByNodeid(a.nodeid) === BACKBONE || _this14.getNodeTypeNameByNodeid(a.nodeid) === L3_SWITCH || _this14.getNodeTypeNameByNodeid(a.nodeid) === L2_SWITCH) && b.nodeid && _this14.getNodeTypeNameByNodeid(b.nodeid) === PATCH_PANEL && c.category_id) {
                            //if((a.x+1 === c.x) && (c.x+1 === b.x))
                            {
                                break;
                            }
                    ...
                    if (foundswitchtopo) {
                        ...
                        if (util.checkforvalidity(tempremaining)) {
                            list = tempremaining;
                            util.addInvalid(invalid, tempinvalid);
                            for (var _i4 = 0; _i4 < tempinvalid.length; _i4++) {
                                if (tempinvalid[_i4].nodeid) _this14._addPatchPanelList(patchpanels, tempinvalid[_i4].nodeid);
                            }
                        } else {
                            util.addInvalid(invalid, srcList);
                            for (var _i5 = 0; _i5 < list.length; _i5++) {
                                if (list[_i5].nodeid) _this14._addPatchPanelList(patchpanels, list[_i5].nodeid);
                            }
                            return;
                        }
                    } else {
                        ...
                        for (var _i6 = 0; _i6 < list.length; _i6++) {
                            if (list[_i6].nodeid) _this14._addPatchPanelList(patchpanels, list[_i6].nodeid);
                        }
                        return;
                    }
                }

                for (var _i7 = 0; _i7 + 2 < list.length;) {
                    var _a = list[_i7],
                        _c = list[_i7 + 1],
                        _b = list[_i7 + 2];
                    if (_a.nodeid && _b.nodeid) {
                        _this14._addPatchPanelList(patchpanels, _a.nodeid);
                        _this14._addPatchPanelList(patchpanels, _b.nodeid);
                        linkedelements.push({
                            interface_a: { nodeid: _a.nodeid, portname: _a.portname },
                            connector: { category_id: _c.category_id },
                            interface_b: { nodeid: _b.nodeid, portname: _b.portname }
            ...
            //if (linkedelements.length === 0 || patchpanels.length === 0) return;
            //if (patchpanels.length === 0) return;
            if (isBasis) {
                this.buildLinkageByElements(linkedelements, invalid);
                this.dirty = true;
            } else {
                if (patchpanels.length === 0) return;
                this.handleBulkImportSave(linkedelements, patchpanels, invalid, switchtopos);
            }
        }
    }, {
        key: 'handleBulkImportSave',
        value: function handleBulkImportSave(linkedelements, patchpanels, invalid, switchtopos) {
            var data = {
                linkedelements: linkedelements,
                patchpanels: patchpanels.map(function (d) {
                    return { nodeid: d };
                }),
                invalid: invalid,
                switchtopo: switchtopos
            };
            new Promise(function (resolve, reject) {
                util.ajax({
                    url: API_HEAD + 'getLinkageConnectionModelData/' + patchpanels[0],
                    success: function success(r) {
                        if (r.id) {
                            data.id = r.id;
                        }
        ...
        value: function _addPatchPanelList(list, nodeid) {
            if (list.indexOf(nodeid) < 0) {
                var pn = this.getNodeTypeNameByNodeid(nodeid);
                if (pn === PATCH_PANEL) {
                    list.push(nodeid);
                }
            }
        ...
        key: 'createCable',
        value: function createCable(data) {
            var node = new ht.Node();
            node.setImage(this.getCableLinkageImage(data.getImage()));
            node.a({
                'device.type': 2,
                'category_id': data.a('id')
            });
        ...
        value: function getPortLinkageImage(type) {
            var img = this.linkageImgMap[type];
            if (img) {
                return {
                    "dataBindings": [{
                        "attr": "device.name",
        ...
        key: 'createDevicePort',
        value: function createDevicePort(data, name, port) {
            var node = new ht.Node(),
                img = this.getPortLinkageImage(this.getNodeTypeByNodeid(data.a('nodeid')));
            node.setImage(img);
            node.a({
        ...
        value: function createDevicePort2(attr, name, port) {
            var node = new ht.Node(),
                img = this.getPortLinkageImage(this.getNodeTypeByNodeid(attr.nodeid));
            node.setImage(img);
        ...
        key: 'parseCSVData',
        value: function parseCSVData(list, cable, deviceName, devicePort) {
            if (!(cable && deviceName && devicePort && !isNaN(devicePort))) return false;

            var nodeid = this.getNodeidByDeviceName(deviceName),
                cableData = this.getCableByCIID(cable),
                customId = nodeid + '-' + devicePort,
                dp = this.getNodeOnTreeByCustomId(customId),
                plugable = dp ? !dp.a('isPlugged') : false;

            if (!nodeid || !cableData || !plugable) return false;

            list.push({ type: 2, cable: cableData });
            list.push({ type: 1, port: parseInt(devicePort), customId: customId, nodeid: nodeid });
            return true;
        }
    }, {
        key: 'importByCSV',
        value: function importByCSV() {
            ...
            util.selectFile(null, 'text', function (file, text) {
                _this15.readCSV(text);
                _this15.gv.fitContent(1);
            });
        }
    }, {
        key: 'switchPatchPanel',
        value: function switchPatchPanel(patch) {
            var _this16 = this;

            nodeid = util.getNodeId(patch);
            this.loadDevicePorts();
            this.loadPatchPanel();
            setTimeout(function () {
                _this16.resetTreePortAvailable();
            }, 1500);
        }
    }, {
        key: 'dirty',
