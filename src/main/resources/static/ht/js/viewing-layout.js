...
var CompatibleWidgetList = function (_ht$widget$BaseDropDo) {
    inherits(CompatibleWidgetList, _ht$widget$BaseDropDo);

    function CompatibleWidgetList(master) {
        classCallCheck(this, CompatibleWidgetList);

        var _this = possibleConstructorReturn(this, (CompatibleWidgetList.__proto__ || Object.getPrototypeOf(CompatibleWidgetList)).call(this, master));

        var listView = _this.listView = new ht.widget.ListView();
        ...
        listView.getLabel = function (data) {
            return data.a('title') ? data.a('title') : data.a('mainpage_name');
            //return data.a('mainpage_name');
        };
    ...
    createClass(CompatibleWidgetList, [{
        key: 'loadListDatas',
        value: function loadListDatas() {
            var _this2 = this;

            loading.show(this.getView());
            var widget = cms.gv.sm().ld();
            if (!widget) return;

            util.ajax({
                url: API_HEAD + 'getCompatibleWidgetForLayout?samplewidget_id=' + widget.a('sampleWidgetId') + '&widget_id=' + widget.a('widgetId'),
                type: 'get',
                success: function success(r) {
                    loading.hide();
                    var dm = _this2.listView.dm();
                    ...
                    r.forEach(function (d) {
                        ...
                        data.a(d);
                        data.setToolTip('<p>MainPage Name: ' + d.mainpage_name + '</p>\n                        <p>Title:  ' + d.title + '</p>');
                        dm.add(data);
                    });
                },
        ...
        value: function getValue() {
            ...
            if (ld) {
                if (widget) {
                    widget.a({
                        'widgetId': ld ? ld.a('widget_id') : '',
                        'title': ld ? ld.a('title') : '',
                        compatibleName: ld ? ld.a('title') : ''
                    });

                    cms.handleClickWidget();
                }
                return ld.a('title');
            } else {
                return '';
    ...
    return CompatibleWidgetList;
}(ht.widget.BaseDropDownTemplate);

var WIDGET_TYPE_MAP = void 0;
var mainPageId = util.getUrlParamter('mainpageid');

util.ajax({
    url: API_HEAD + 'getMainpageWidgetTypeIDByName/Map',
    success: function success(r) {
        WIDGET_TYPE_MAP = r;
    }
});
...
var CMS = function () {
    ...
    createClass(CMS, [{
        ...
        value: function handleClickWidget() {
            ...
            if (!data) {
                this.formPane.reset();
                this.formPane.getView().style.display = 'none';
                return;
            }
            this.formPane.getView().style.display = 'block';
            this.formPane.iv();
            var csp = data.s('attach.column.span') || 1,
                rsp = data.s('attach.row.span') || 1,
                size = csp + 'X' + rsp;
            this.formPane.setValues({
                title: data.a('title') || '',
                colIndex: data.s('attach.column.index'),
                rowIndex: data.s('attach.row.index'),
                size: size,
                compatible: data.a('compatibleName')
            });
            var types = this.widgetTypeMap[size];
            if (types) {
                var comboBox = this.formPane.getItemById('widget_type').element;
                this.setWidgetTypeVL(types, data.a('widgetType'));
            } else {
                this.loadWidgetType(size, data.a('widgetType'));
            }
        }
    }, {
        key: 'loadWidgetType',
        value: function loadWidgetType(size, val) {
            var _this = this;

            loading.show(this.formPane.getView());
            util.ajax({
                url: API_HEAD + 'getAvailableWidgetForLayoutSize/' + size,
                type: 'get',
                success: function success(r) {
                    loading.hide();
                    // let  comboBox = this.formPane.getItemById('widget_type').element;
                    // comboBox.setValues(r);
                    _this.widgetTypeMap[size] = r;
                    _this.setWidgetTypeVL(r, val);
                },
                error: function error(s) {
                    console.log(s);
                }
        ...
        key: 'setWidgetTypeVL',
        value: function setWidgetTypeVL(r, val) {
        ...
        value: function initUI() {
            ...
            var self = this,
                rightTopToolbar = this.rightTopToolbar = new Toolbar(32),
            ...
            formPane.getView().style.display = 'none';

            //rightBorderPane.getView().style.borderLeft = '1px solid #999';
            rightBorderPane.getView().style.border = '1px solid grey';
            rightBorderPane.getView().style.borderRadius = '5px';

            rightBorderPane.setCenterView(formPane);
            rightBorderPane.setTopView(rightTopToolbar);
            rightBorderPane.setTopHeight(rightTopToolbar.toolbarHeight);

            gv.getView().style.border = '1px solid grey';
            gv.getView().style.borderRadius = '5px';

            borderPane.setCenterView(gv);
            borderPane.setRightView(rightBorderPane);
            borderPane.setRightWidth(270);
            borderPane.addToDOM();

            gv.setInteractors([new ht.graph.SelectInteractor(gv)]);
            gv.sm().ms(this.handleClickWidget, this);
            gv.addTopPainter(this.drawReadyFlag.bind(this));

            var menuItems = [{
                label: 'Manage',
                action: function action() {
                    var ld = _this2.gv.sm().ld(),
                        colindex = ld.s('attach.column.index') || 0,
                        rowindex = ld.s('attach.row.index') || 0;
                    if (ld.a('widgetType') === WIDGET_TYPE_MAP) {
                        // map
                        var url = 'map-widget.html?mainpageid=' + mainPageId + '&colindex=' + colindex + '&rowindex=' + rowindex + '&widgetType=' + ld.a('widgetType');
                        if (ld.a('widgetId')) {
                            url += '&id=' + ld.a('widgetId');
                        }
                        location.href = url;
                    } else {
                        // non map
                        location.href = '/admin/setting/manage-widget.html?id=' + mainPageId + '&rowindex=' + ld.s('attach.row.index') + '&colindex=' + ld.s('attach.column.index');
                    }
                }
            }, {
                label: 'Reset',
                ...
                action: function action() {
                    var ld = _this2.gv.sm().ld();
                    if (!ld.a('widgetId')) return;
                    loading.show();
                    util.ajax({
                        url: API_HEAD + 'resetMainPageWidget/' + ld.a('widgetId') + '?mainpageid=' + mainPageId,
                        success: function success(r) {
                            loading.hide();
                            ld.a('widgetId', null);
                            _this2.handleSave();
            ...
            contextMenu.beforeShow = function (e) {
                var widget = self.gv.getDataAt(e);
                if (!widget) {
                    this.setItems([]);
                } else {
        ...
        value: function initForm() {
            ...
            multiComboBox.setDropDownComponent(CompatibleWidgetList);
            ...
            var formItems = [{
                ...
                comboBox: {
                    onValueChanged: function onValueChanged(oldVal, newVal) {
                        var ld = _this3.gv.sm().ld(),
                            samples = _this3.currSamples;
                        if (ld && newVal && newVal !== ld.a('widgetType')) {
                            ld.a('widgetType', newVal);
                            ld.a('title', '');
                            ld.a('widgetId', null);
                            for (var i = 0; i < samples.length; i++) {
                                if (samples[i].id === newVal) {
                                    ld.setImage(samples[i].img);
                                    ld.a('sampleWidgetId', samples[i].samplewidget_id);
                                    ld.a('name', samples[i].name);
                                    break;
                                }
                            }
                        }
                ...
                label: 'Compatible Widget',
                id: 'compatible',
                element: multiComboBox
            }];
            this.formPane.addFormItems(formItems, [108, 0.1], 26);

            var titleInput = this.formPane.getItemById('title').element.getElement();
            titleInput.addEventListener('keyup', function (e) {
                var ld = _this3.gv.sm().ld();
                if (ld) {
                    ld.a('title', titleInput.value);
                }
            });
        }
    }, {
        ...
        value: function initToolbar() {
            ...
            this.rightTopToolbar.addRow(['', {
                button: {
                    icon: 'editor.preview',
                    toolTip: 'Preview',
                    onClicked: function onClicked(e) {
                        window.open('/admin/setting/preview-layout.html?id=' + mainPageId);
                    }
                }
            }, {
                button: {
        ...
        value: function serialize() {
            return this.widgetLayout.serialize();
        }
    }, {
        key: 'loadMainPageLayoutDetail',
        value: function loadMainPageLayoutDetail() {
            var _this5 = this;

            if (!mainPageId) return;
            loading.show();
            util.ajax({
                url: API_HEAD + 'getMainPageLayoutDetail/' + mainPageId,
                success: function success(r) {
                    loading.hide();
                    if (r) {
                        _this5.initGrid(r.mainpage);
                        r.widgets.forEach(function (w) {
                            _this5.gv.dm().each(function (node) {
                                var ci = node.s('attach.column.index') || 0,
                                    ri = node.s('attach.row.index') || 0;
                                if (ci === w.colindex && ri === w.rowindex) {
                                    if (w.widget_id) {
                                        node.a('widgetId', w.widget_id);
                                    }
                                    if (w.title) {
                                        node.a('title', w.title);
                                    }
                                }
                            });
                        });
                        //this.afterBuildMapWidget();
                    }
                }
            });
        }
        ...
        key: 'initGrid',
        value: function initGrid(mainPage) {
            if (mainPage.json_data) {
                var gv = this.gv;
                gv.dm().deserialize(mainPage.json_data);

                var gl = gv.dm().toDatas(function (d) {
        ...
        key: 'afterBuildMapWidget',
        value: function afterBuildMapWidget() {
            var storage = localStorage.getItem('mapWidgetBuildInfo');
            if (storage) {
                ...
                if (info.mainPageId !== mainPageId) return;

                this.gv.dm().each(function (d) {
                    var ci = d.s('attach.column.index'),
                        ri = d.s('attach.row.index');
                    if (rowindex === ri && colindex === ci && !(d instanceof ht.Grid)) {
                        d.a('widgetId', widgetId);
                        d.a('compatibleName', null);
                        localStorage.removeItem('mapWidgetBuildInfo');
                    }
                });
            }
        }
        ...
        key: 'gridFitGv',
        value: function gridFitGv() {
            this.grid.setSize(this.gv.getWidth(), this.gv.getHeight());

            this.gv.fitContent(1, 0);
        }
    }, {
        key: 'handleSave',
        value: function handleSave() {
            var widgets = this.grid.getAttaches(),
                data = {
            ...
            widgets.each(function (w) {
                var obj = {
                    title: w.a('title') || '',
                    colindex: w.s('attach.column.index'),
                    rowindex: w.s('attach.row.index'),
                    samplewidget_id: w.a('sampleWidgetId')
                };
                if (w.a('widgetId')) {
                    obj.widget_id = w.a('widgetId');
                }
                data.widgetList.push(obj);
            });
            loading.show();
            util.ajax({
                url: API_HEAD + 'saveMainPageLayoutDetail/' + mainPageId,
                type: 'post',
                dataType: 'json',
        ...
        key: 'drawReadyFlag',
        value: function drawReadyFlag(g) {
            if (!this.grid) {
                return;
            }
            var widgets = this.grid.getAttaches();
            ...
            var img = ht.Default.getImage('ready.flag');
            widgets.each(function (w) {
                if (w.a('widgetId')) {
                    var rect = w.getRect(),
                        x = rect.x,
                    ...
                    ht.Default.drawImage(g, img, x + (width - img.width) * 0.5, y + (height - img.height) * 0.5, img.width, img.height);
                }
            });
        }
    }, {
        key: 'manageComplete',
        value: function manageComplete(ci, ri, widgetId) {
            var datas = this.gv.dm().toDatas(function (d) {
                return !(d instanceof ht.Grid);
            }),
                size = datas.size();
            for (var i = 0; i < size; i++) {
                var node = datas.get(i),
                    rowIndex = node.s('attach.row.index') || 0,
                    columnIndex = node.s('attach.column.index') || 0;
                if (columnIndex === ci && rowIndex === ri) {
                    node.a('widgetId', widgetId);
                    break;
                }
            }
        }
