...
var NodeAlarm = function () {
    ...
    createClass(NodeAlarm, [{
        ...
        value: function loadNodeStatus() {
            ...
            if (this.targetId) {
                util.ajax({
                    url: API_HEAD + 'getAlarmStatusOfMapWidget/' + this.targetId,
                    success: function success(r) {

                        if (r && r.length > 0) {
                            r.forEach(function (item) {
                                var targetList = _this2._dm.toDatas();
                                for (var i = 0; i < targetList.size(); i++) {
                                    if (targetList.get(i).a('deviceName') + '' === item.devicename) {
                                        _this2.startNodeAlarm(targetList.get(i), item.status);
                                        break;
                                    }
                                }
                            });
...
var ICON_SIZE = 24;
var MAP_WIDGET = 'MapWidget';
var btnIconColor = 'rgb(159, 159, 159)';
var btnSelectColor = 'rgb(231, 231, 231)';
var btnBgColor = '#fff';

var mainPageId = parseInt(util.getUrlParamter('mainpageid'));
var colindex = parseInt(util.getUrlParamter('colindex'));
var rowindex = parseInt(util.getUrlParamter('rowindex'));
...
var CMS = function () {
    function CMS() {
        ...
        this.widgetId = parseInt(util.getUrlParamter('id')) || null;
        this.init();
    }
    ...
    createClass(CMS, [{
        ...
        value: function initUI() {
            ...
            var gv = this.gv = new ht.graph.GraphView(),
                ...
                leftSplitPane = this.leftSplitPane = new ht.widget.SplitView(leftTopBorderPane, propertyForm, 'v', 0.56),
            ...
            leftTopBorderPaneInner.setTopHeight(46);
            ...
            leftTopBorderPane.setTopHeight(100);
        ...
        value: function initMapWidget() {
            ...
            if (!this.widgetId) {
                ...
                loading.show();
                util.ajax({
                    url: API_HEAD + 'getMapWidgetModelData/' + this.widgetId,
                    success: function success(r) {
                        loading.hide();
        ...
        value: function loadIconData() {
            ...
            dm.add(group);
            var t = ["AAA", "Auth_lvl", "BackupSystem", "F5", "FA", "Firewall1", "Firewall2", "Firewall3", "Firewall4", "Firewall5", "Firewall6", "Firewall7", "FirewallGE", "VoiceRouter", "VPN1", "VPN2"];
        ...
        value: function getOPMDeviceType() {
            var self = this;
            //let listForm = self.listForm, deviceTypes = self.deviceTypes;

            util.ajax({
                url: API_HEAD + 'getOPMDeviceType',
                type: 'get',
                success: function success(r) {
                    var combobox = self.listForm.getItemById('deviceType').element,
                        labels = ['All'],
                        values = ['all'];

                    self.deviceTypes = JSON.parse(JSON.stringify(r));
                    for (var key in r) {
                        labels.push(r[key]);
                        values.push(key);
                    }
        ...
        key: 'removeSubMap',
        value: function removeSubMap(data) {
            var self = this;
            var url = API_HEAD + 'deleteMapWidget';

            if (data.a('deviceName')) {
                url += '/' + data.a('deviceName');
            }
        ...
        value: function initMapPropertyForm() {
            ...
            //form.addSpaceRow(10);
            form.addRow([{ element: 'MapWidget Property:', font: 'bold 12px arial, sans-serif' }], [0.1], 30, { background: '#f7f7f7' });
            form.addRow([], [0.1], 1.01, { background: '#43AFF1' });
            form.addRow([// map Name
            'Display Name', {
                id: 'displayName',
                getter: function getter() {
                    return _this2.bgNode.a('displayName');
                },
                setter: function setter(v) {
                    _this2.bgNode.a('displayName', v);
                },
                textField: {}
            }], colWidth);
            form.addRow([// width
            ...
            */
            ...
            form.addRow(['Alarm Animate', {
                button: {
                    ...
                    onClicked: function onClicked() {
                        if (self.widgetId) {
                            self.nodeAlarm.toggle(self.widgetId);
        ...
        value: function initPropertyView() {
            ...
            var nodeProperties = [{
                ...
                setValue: function setValue(data, property, value, view) {
                    if (value === 'default') {
                        data.setImage(_this3.getProductImageById(data.a('productId')));
            ...
            this.propertyView.getView().addEventListener('click', function (e) {
                var div = _this3.propertyView.getView(),
                    top = div.offsetTop,
                    offsetY = e.clientY - top,
                    rowHeight = _this3.propertyView.getRowHeight(),
                    rowIndex = Math.floor(offsetY / rowHeight),
                    row = _this3.propertyView.getRows().get(rowIndex);
            ...
            this.gv.sm().ms(function () {
                ...
                if (ld instanceof ht.Edge) {
                    ...
                    _this3.propertyView.setProperties(edgeProperties);
                    _this3.propertyView.getView().style.height = '420px';
                    _this3.propertyView.getView().style.display = 'block';
                    setTimeout(function () {
                        _this3.getEdgeDevices(ld);
                    }, 100);
                } else if (ld instanceof ht.Node && ld !== _this3.bgNode) {
                    _this3.propertyView.setProperties(nodeProperties);
                    _this3.propertyView.getView().style.height = '280px';
                    _this3.propertyView.getView().style.display = 'block';

                    if (!ld.s('label.position')) {
                        ld.s('label.position', 31);
                    }
                    if (!ld.s('label.rotation')) {
                        ld.s('label.rotation', 0);
                    }
                    if (!ld.s('label.font')) {
            ...
            dialog.setConfig({
                ...
                buttons: [{
                    ...
                    action: function action(button, e) {
                        _this4.handleSelectProductType();
                    }
                }, {
                    label: 'Cancel',
                    action: function action() {
                        _this4.dm.remove(_this4._lastAdd);
                        _this4._lastAdd = null;
                        _this4.listView.ivm();
                        dialog.hide();
                    }
                }],
        ...
        value: function initToolbar() {
            ...
            if (!isNaN(mainPageId) && !isNaN(colindex) && !isNaN(rowindex)) {
                toolbar.addRow(['', this.createToobarBtn({
                    ...
                    toolTip: 'Edge',
                    onClicked: function onClicked() {
                        _this5.gv.setInteractors(_this5.edgeEditInteractors);
                    }
                }), '', this.createToobarBtn({
                ...
                this.createToobarBtn({
                    icon: this.getToolbarIcon('editor.preview'),
                    toolTip: 'Save MapWidget Only',
                    onClicked: function onClicked() {
                        if (_this5.propertyForm.v('displayName') === 'undefined') {
                            alert('Please change Display Name');
                        } else {
                            _this5.handleSaveMapWidget(function (r) {

                                if (!_this5.widgetId) {
                                    _this5.widgetId = r.id;
                                }
                ...
                '', this.createToobarBtn({
                    ...
                    icon: this.getToolbarIcon('toolbar.save'),
                    toolTip: 'Accept MapWidget',
                    onClicked: function onClicked() {
                        if (_this5.propertyForm.v('displayName') === 'undefined') {
                            alert('Please change Display Name');
                        } else {
                            _this5.handleSave(function (r) {
                                window.top.location.href = '/admin/setting/manage-layout?id=' + mainPageId;
                            });
                        }
                    }
                }), ''], [0.1, 32, 32, 12, 32, 32, 12, 32, 12, 32, 12, 32, 0.1], 32);
            } else {
                toolbar.addRow(['', this.createToobarBtn({
                    icon: this.getToolbarIcon('editor.edit'),
                    togglable: true,
        ...
        key: 'handleSave',
        value: function handleSave(callback) {
            var content = this.dm.serialize().replace(/\n+/g, ''),
                url = API_HEAD + 'saveMapWidget';
            var data = {
        ...
        key: 'handleSaveMapWidget',
        value: function handleSaveMapWidget(callback) {
            var content = this.dm.serialize().replace(/\n+/g, ''),
                url = API_HEAD + 'saveMapWidgetOnly';
            var data = {
                display_name: this.propertyForm.v('displayName'),
                content_data: content,
                option_data: '',
        ...
        value: function getDevices() {
            ...
            this.dm.each(function (d) {
                var category = d.a('category');
                if (category && category !== 'MapWidget') {
        ...
        key: 'getSubMapWidgets',
        value: function getSubMapWidgets() {
            var list = [];
            this.dm.each(function (d) {
                var category = d.a('category');
                if (category && category === 'MapWidget') {
        ...
        value: function getEdgeDevices(edge) {
            this.updateEdgeDevices([edge.getSource().a('deviceName'), edge.getTarget().a('deviceName')]);
        }
    }, {
        ...
        value: function handleListDND(e, state) {
            ...
            if (state === 'begin') {
                ...
                if (ht.Default.containedInView(e, this.gv)) {
                    ...
                    node.p(this.gv.lp(e));
                    if (sd.a('category') === MAP_WIDGET) {
                        node.setImage('./imgs/cisco/building.png');
                    } else {
                        var imgname = undefined;
                        for (var key in deviceTypes) {
                            if (sd.a('category') === deviceTypes[key]) {
                                imgname = key;
                                break;
                            }
                        }
        ...
        value: function loadListDatas() {
            var self = this,
                url = API_HEAD + 'searchDeviceListForMapWidget',
                deviceType = self.listForm.v('deviceType'),
                keyword = self.listForm.v('searchKeyword');

            if (deviceType === 'Select Type') {
                deviceType = 'all';
            }
            url += '/' + deviceType;
            if (keyword) {
