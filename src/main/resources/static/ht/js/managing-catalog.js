...
var DND = function () {
    ...
    createClass(DND, [{
        ...
        key: 'showTip',
        value: function showTip(text, event) {
            var tip = this.tip,
                point = ht.Default.getPagePoint(event);

            tip.innerHTML = text;
            if (tip.parentNode !== document.body) {
                document.body.appendChild(tip);
            }
            tip.style.left = point.x + 15 + 'px';
        ...
        get: function get$$1() {
            if (!this._tip) {
                this._tip = ht.Default.createDiv();
...
var drawBorder = ht.Default.drawBorder;
var PARENT = 'parent';
var UP = 'up';
var DOWN = 'down';

var DNDTree = function (_ht$widget$TreeView) {
    inherits(DNDTree, _ht$widget$TreeView);

    function DNDTree(dm) {
        classCallCheck(this, DNDTree);

        var _this = possibleConstructorReturn(this, (DNDTree.__proto__ || Object.getPrototypeOf(DNDTree)).call(this, dm));

        _this._draggable = true;
        _this.addTopPainter(_this.drawDNDState.bind(_this));
        return _this;
    }

    createClass(DNDTree, [{
        key: '_clearDragInfo',
        value: function _clearDragInfo() {
            this.dnd.hideTip();
            if (!this._dragInfo) {
                return;
            }
        ...
        value: function _dragging(e) {
            ...
            var rowHeight = this.getRowHeight(),
                point = this.lp(e),
                rowIndex = Math.floor(point.y / rowHeight),
                size = this.getRowSize();

            if (rowIndex < size) {
                var delta = point.y - rowIndex * rowHeight;
                if (delta < rowHeight * 0.25) {
                    dragInfo.type = UP;
                } else if (delta > rowHeight * 0.75) {
                    dragInfo.type = DOWN;
                } else {
                    dragInfo.type = PARENT;
                }
                var refData = dragInfo.refData = this.getDataAt(e);
                if (dragInfo.type === UP) {
                    dragInfo.parent = refData.getParent();
                    dragInfo.x = (this.getLevel(refData) + 1) * this.getIndent();
                    dragInfo.y = rowIndex * rowHeight;
                } else if (dragInfo.type === DOWN) {
                    if (this.getToggleIcon(refData) === this.getExpandIcon()) {
                        dragInfo.parent = refData;
                        dragInfo.x = (this.getLevel(refData) + 2) * this.getIndent();
                    } else {
                        dragInfo.parent = refData.getParent();
                        dragInfo.x = (this.getLevel(refData) + 1) * this.getIndent();
                    }
                    dragInfo.y = (rowIndex + 1) * rowHeight;
                } else {
                    dragInfo.parent = refData;
                }
            } else {
                dragInfo.refData = null;
                dragInfo.type = PARENT;
                dragInfo.parent = null;
            }
            this.autoScroll(event);
            this.redraw();
            dnd.showTip(dragInfo.target.getName(), e);
        }
    }, {
        key: 'drawDNDState',
        value: function drawDNDState(g) {
            if (!this._dragInfo || this._dragInfo.target === this._dragInfo.refData) return;
            var dragInfo = this._dragInfo;
            ...
            var parent = dragInfo.parent;
            var parentIndex = parent ? this.getRowDatas().indexOf(parent) : -1;
            var colorSelectDark = theme.colorSelectDark;


            if (dragInfo.refData) {
                drawBorder(g, colorSelectDark, 0, parentIndex * rowHeight, width, rowHeight, 2);
                if (type === UP || type === DOWN) {
                    var x = dragInfo.x;
                    var y = dragInfo.y;
                    g.beginPath();
                    g.arc(x - 3, y, 3, 0, Math.PI * 2, true);
                    g.moveTo(x, y);
                    g.lineTo(width, y);
                    g.lineWidth = 2;
                    g.strokeStyle = colorSelectDark;
                    g.stroke();
                }
            } else {
                var rect = this.getViewRect();
                drawBorder(g, colorSelectDark, rect.x, rect.y, rect.width, rect.height, 2);
            }
        }
    }, {
        key: 'handleDragAndDrop',
        value: function handleDragAndDrop(e, state) {
            ...
            if (state === 'begin') {
                ...
                var dragInfo = this._dragInfo;
                var inView = this.getView().contains(ht.Default.getTargetElement(e));
                if (dragInfo && inView) {
                    var _target = dragInfo.target,
                        type = dragInfo.type,
                        parent = dragInfo.parent,
                        refData = dragInfo.refData;

                    _target.setParent(null);
                    _target.setParent(parent);
                    if (dragInfo.type === UP || dragInfo.type === DOWN) {
                        var index = parent.getChildren().indexOf(refData);
                        if (dragInfo.type === DOWN) {
                            index++;
                        }
                        this.dm().moveTo(_target, index);
                    }
                }
                this._clearDragInfo();
            }
...
var CREATE = TREE_EDIT_MODE.CREATE;
var EDIT = TREE_EDIT_MODE.EDIT;
...
var CMS = function () {
    ...
    createClass(CMS, [{
        ...
        value: function initUI() {
            ...
            var self = this,
                treeView = this.treeView = new DNDTree(),
                contextMenu = this.contextMenu = new ht.widget.ContextMenu(),
                toolbar = this.toolbar = new Toolbar(24, 0, 12),
                borderPane = this.borderPane = new ht.widget.BorderPane(),
                TITLE_CREATE = 'New Node',
                TITLE_EDIT = 'Modify Node';
            treeView.setDraggable(false);
            this.tipWin = tipWin;

            treeView._selectBackground = theme.themeColor;
            ...
            borderPane.setTopHeight(toolbar.toolbarHeight);
            borderPane.addToDOM();
            ...
            contextMenu.setItems([{
                ...
                action: function action() {
                    self.editDialog.setTitle(TITLE_CREATE);
                    self.editForm.reset();
                    self.clearFormError();
                    self.dialogMode = CREATE;
                ...
                action: function action() {
                    var ld = self.treeView.sm().ld();
                    self.editDialog.setTitle(TITLE_EDIT);
                    self.clearFormError();
                    _this.updateFormItemElement(ld.a('nodeType'));
                    self.editForm.v('name', ld.getName());
                    self.editForm.v('nodeType', ld.a('nodeType'));
                    self.editForm.v('icon', ld.a('icon'));
        ...
        key: 'updateFormItemElement',
        value: function updateFormItemElement(nodeType) {
            this.editForm.updateItemElement('name', nodeType === 1 ? this.nameTextField : this.nameComboBox);
        }
    }, {
        key: 'initDialog',
        value: function initDialog() {
            ...
            var productTypes = this.productTypes,
                ...
                editDialog = this.editDialog = new ht.widget.Dialog(),
                editForm = this.editForm = new FormPane(),
                itemSpan = [80, 0.1];


            var nameComboBox = this.nameComboBox = new ht.widget.ComboBox();
            nameComboBox.setValues(productTypes.map(function (d) {
                return d.name;
            }));
            nameComboBox.onValueChanged = function (oldVal, newVal) {
                for (var i = 0; i < productTypes.length; i++) {
                    if (productTypes[i].name === newVal) {
                        editForm.v('productTypeId', productTypes[i].id);
                        break;
                    }
                }
            };
            // nameComboBox.setLabels(productTypes.map(d => d.name));

            var nameTextField = this.nameTextField = new ht.widget.TextField();
            ...
            var formItems = [{
                ...
                value: 1,
                comboBox: {
                    labels: ['Directory', 'Product Type\'s Name'],
                    values: [1, 2],
                    onValueChanged: function onValueChanged(oldVal, newVal) {
                        _this2.updateFormItemElement(newVal);
                    }
                }
            }, {
                id: 'name',
                ...
                id: 'icon',
                label: 'Icon',
                value: 'treeicon-directory',
                comboBox: {
                    labels: ['Directory', 'Rack', 'Device'],
                    icons: ['treeicon-directory', 'tree.icon99', 'tree.icon'],
                    values: ['treeicon-directory', 'tree.icon99', 'tree.icon']
                }
            }];

            editForm.addFormItems(formItems, itemSpan, 24);
            editForm.addRow([{
                id: 'productTypeId',
                textField: {}
            }], [0.1], 0);
            editForm.setVisibleFunc(function (item) {
                return !!item.height;
            });
            editDialog.setConfig({
                title: "New Node",
                content: editForm,
                width: 320,
                height: 220,
        ...
        value: function handleDialogSave() {
            var nodeType = this.editForm.v('nodeType'),
                name = this.editForm.v('name'),
                icon = this.editForm.v('icon'),
                productTypeId = this.editForm.v('productTypeId');
            if (!nodeType) {
                this.editForm.showError('Please select a node type');
                return;
            }
            if (!name) {
                this.editForm.showError('Please enter a name');
                return;
            }
            if (!icon) {
                this.editForm.showError('Please select an icon');
                return;
            }
            if (this.dialogMode === CREATE) {
                this.createTreeNode(name, icon, productTypeId);
            } else {
                var ld = this.treeView.sm().ld();
                ld.a({
                    nodeType: nodeType, icon: icon
                });
                if (nodeType === 2) {
                    ld.a({
        ...
        key: 'clearFormError',
        value: function clearFormError() {
            this.editForm.hideError();
        }
    }, {
        key: 'createTreeNode',
        value: function createTreeNode(name, icon, productTypeId) {
            var ld = this.treeView.sm().ld(),
                data = new ht.Data();
            ...
            data.a({
                icon: icon,
                productTypeId: productTypeId,
                nodeType: this.editForm.v('nodeType')
            });
            if (ld) {
                data.setParent(ld);
            }
            this.treeView.dm().add(data);
        ...
        value: function handleRemove() {
            var ld = this.treeView.sm().ld();
            if (ld) {
                this.treeView.dm().remove(ld);
            }
        }
    }, {
        key: 'loadProductTree',
        value: function loadProductTree() {
            var _this3 = this;

            util.ajax({
                url: API_HEAD + 'getProductTree',
                success: function success(r) {
                    if (!r.content_data) return;
                    var treeData = ht.Default.parse(r.content_data);

                    if (treeData.d) {
                        _this3.dm.deserialize(treeData);

                        _this3.dm.mm(_this3.handleTreeUpdate, _this3);
                        _this3.dm.mh(_this3.handleTreeUpdate, _this3);
                        _this3.dm.md(_this3.handleTreeUpdate, _this3);

                        _this3._notifier.fire({ kind: 'init' });
                    }
                }
            });
        }
        ...
        key: 'loadProductType',
        value: function loadProductType() {
            var _this4 = this;

            util.ajax({
                url: API_HEAD + 'getProductType',
                success: function success(r) {
                    _this4.productTypes = r;
                    _this4.initDialog();
                }
            });
        }
        ...
        value: function save() {
            ...
            var data = this.dm.serialize();
            this.savable = false;
            // str = str.replace(/\n+/g, '');
            util.ajax({
                url: API_HEAD + 'saveProductTree',
                type: 'post',
                dataType: 'text',
                data: JSON.stringify({ content_data: data }),
                success: function success(r) {
