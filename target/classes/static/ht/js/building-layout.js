...
// import GraphPane from '../widget/GraphPane';

var CMS = function () {
    function CMS(mainPageId) {
    ...
    createClass(CMS, [{
        ...
        value: function initUI() {
            ...
            var boderPane = this.borderPane = new ht.widget.BorderPane(),
                gv = this.gv = new ht.graph.GraphView(),
                palette = this.palette = new ht.widget.Palette(),
                mainSplitPane = this.mainSplitPane = new ht.widget.SplitView(this.palette, this.gv, 'h', 430),
                contextMenu = this.contextMenu = new ht.widget.ContextMenu(),
                toolbar = this.toolbar = new ht.widget.FormPane(),
                nextWrap = this.nextWrap = ht.Default.createDiv(),
                nextBtn = ht.Default.toCanvas('preview.next', 32, 32, 'uniform');

            nextWrap.style.width = '32px';
            nextWrap.style.height = '32px';
            nextWrap.style.right = '20px';
            nextWrap.style.top = '100px';
            nextWrap.style.zIndex = 100;
            nextWrap.style.cursor = 'pointer';
            nextWrap.addEventListener('click', function (e) {
                _this.handleSaveAndPreview();
            });
            nextWrap.appendChild(nextBtn);
            gv.getView().appendChild(nextWrap);
            gv.handleScroll = function () {};

            this.gv.setEditable(true);
            var editInteractor = this.gv.getEditInteractor();
        ...
        value: function initToolBar() {
            ...
            toolbar.addRow([{
                ...
                button: {
                    background: null,
                    borderColor: null,
                    icon: 'toolbar.add.circle',
                    toolTip: 'Add new widget',
                    onClicked: function onClicked(e) {
                        _this2.newWidgetForm.reset({
                            colSpan: 1,
                            rowSpan: 1
                        });
                        _this2.dialog.show();
                ...
                button: {
                    toolTip: 'Remove selceting widget',
                    background: null,
                    borderColor: null,
                    icon: 'toolbar.delete.circle',
                    onClicked: function onClicked(e) {
                        _this2.handleRemoveWidget();
                    }
                }
            }], [0.1, 28, 28], 28);

            palette.getView().appendChild(div);
            palette.setItemImageWidth(200);
            palette.setItemImageHeight(140);
            this.mainSplitPane.mp(function (e) {
        ...
        value: function refreshWidgetType() {
            ...
            this._reqTimer = setTimeout(function () {
                var col = form.v('colSpan'),
                    row = form.v('rowSpan'),
                    size = col + 'X' + row,
                    cache = _this3.typeMap[size];

                _this3._reqTimer = null;
                if (cache !== undefined) {
                    _this3.setWidgetTypeVL(cache);
                } else {
                    if (_this3.loadState) {
                        loading.show(_this3.newWidgetForm.getView());
                    }
                    util.ajax({
                        url: API_HEAD + 'getAllWidgetTypeForSize/' + size,
                        type: 'get',
                        success: function success(r) {
                            if (_this3.loadState) {
                                loading.hide();
                            }
                            _this3.typeMap[size] = r;
                            _this3.setWidgetTypeVL(r);
                        },
        ...
        value: function setWidgetTypeVL(r) {
            ...
            comboBox.setValues(r.map(function (d) {
                return d.widgettype_id;
            }));
            comboBox.setLabels(r.map(function (d) {
                return d.widgettype_name;
            }));
        ...
        value: function getWidgetTypeNameById(id) {
            ...
            for (var i = 0; i < types.length; i++) {
                if (types[i].widgettype_id === id) return types[i].widgettype_name;
            }
        }
    }, {
        ...
        value: function handleSaveNewWidget() {
            ...
            formData.name = formData.colSpan + 'X' + formData.rowSpan + ' ' + this.getWidgetTypeNameById(formData.widget_type);

            self.dialog.hide();
            util.ajax({
                url: API_HEAD + 'insertWidgetForBuildingLayout',
                type: 'post',
                data: formData,
                success: function success(r) {
                    _this4.addPaletteNode(r);
                },
                error: function error(s) {
                    console.log(s);
                }
        ...
        value: function initGrid(mainPage) {
            if (mainPage.json_data) {
                this.gv.dm().deserialize(mainPage.json_data);
                this.gv.fitContent(1);
                var gl = this.gv.dm().toDatas(function (d) {
                    return d instanceof ht.Grid;
                });
                if (gl.size() > 0) {
                    this.grid = gl.get(0);
            ...
            if (mainPage.layout_name) {
                var rc = mainPage.layout_name.split('X'),
                    GRID_COLUMN = rc[0],
                    GRID_ROW = rc[1],
                    GRID_BORDER = 1,
                    GRID_GAP = 0,
        ...
        value: function loadWidgets() {
            ...
            loading.show();
            util.ajax({
                url: API_HEAD + 'getAvailableWidgetForBuildingLayout',
                type: 'get',
                success: function success(r) {
                    var data = r;
                    ...
                    _this5.palette.dm().add(group);
                    _this5._paletteBaseGroup = group;
                    data.forEach(function (d, i) {
                        _this5.addPaletteNode(d);
                    });
                    _this5.loadState = { widget: true };
                },
                error: function error(s) {
                    console.log(s);
                }
        ...
        value: function loadMainPageLayoutDetail() {
            ...
            loading.show();
            util.ajax({
                url: API_HEAD + 'getMainPageLayoutDetail/' + this.mainPageId,
                success: function success(r) {
                    _this6.loadState = { mainPage: true };
                    if (r) {
                        _this6.initGrid(r.mainpage);
                    }
                }
            });
        }
        ...
        value: function handleRemoveWidget() {
            var ld = this.palette.sm().ld();
            if (ld && confirm('Are you sure you want to delete the selected widget?')) {
                var id = ld.a('sampleWidgetId');
                this.palette.dm().remove(ld);
                util.ajax({
                    url: API_HEAD + 'deleteWidgetForBuildingLayout/' + id,
                    success: function success(r) {}
                });
            }
        }
    }, {
        key: 'handleSaveAndPreview',
        value: function handleSaveAndPreview() {
            var _this7 = this;

            // check all filled

            if (!this.checkIsAllFilled()) {
                tipWin.show('Please fill all grid');
                return;
            }
            ...
            loading.show();
            util.ajax({
                url: API_HEAD + 'saveMainPageLayoutDetail/' + this.mainPageId,
                type: 'post',
                dataType: 'json',
                data: JSON.stringify(data),
                success: function success(r) {
                    window.top.location.href = '/admin/setting/manage-layout?id=' + _this7.mainPageId;
                    //location.href = `viewing-layout.html?mainpageid=${this.mainPageId}`;
                }
            });
        }
    }, {
        key: 'checkIsAllFilled',
        value: function checkIsAllFilled() {
            var grid = this.grid,
                rowCount = parseInt(grid.s('grid.row.count')),
                colCount = parseInt(grid.s('grid.column.count')),
                widgets = grid.getAttaches(),
                cl = [];
            if (!widgets) {
                return false;
            }
            widgets.each(function (w) {
                var ci = w.s('attach.column.index'),
                    cp = w.s('attach.column.span'),
                    ri = w.s('attach.row.index'),
                    rp = w.s('attach.row.span');
                for (var i = ci; i <= ci + cp - 1; i++) {
                    for (var j = ri; j <= ri + rp - 1; j++) {
                        var k = i + '-' + j;
                        if (cl.indexOf(k) < 0) {
                            cl.push(k);
                        }
            ...
            return cl.length === rowCount * colCount;
        }
    }, {
        key: 'loadState',
        set: function set$$1(v) {
            ...
            for (var key in v) {
                this._loadState[key] = v[key];
            }
            if (this._loadState.widget && this._loadState.mainPage) {
                loading.hide();
            }
        },
        get: function get$$1() {
            return this._loadState && this._loadState.widget && this._loadState.mainPage;
        }
    }]);
    return CMS;
}();

window.init = function (mainPageId) {
    window.cms = new CMS(mainPageId);
};
